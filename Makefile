...
linux: prepare $(BUILD)/libderWindow.a
linux-ccj:
	$(CMAKE) $(CMAKE_GEN_FLAGS) -DWANDROID=FALSE -DCMAKE_EXPORT_COMPILE_COMMANDS=TRUE
	cp $(BUILD)/compile_commands.json .

android: prepare $(BUILD)/arm64-v8a/libderWindow.a
android-ccj:
...
$(BUILD)/libderWindow.a:
	$(CMAKE) $(CMAKE_GEN_FLAGS) -DWANDROID=FALSE
	$(CMAKE) --build build

# Android Build
$(BUILD)/arm64-v8a/libderWindow.a:
	$(CMAKE) $(CMAKE_GEN_FLAGS) \
	-DWANDROID=TRUE \
	-DANDROID_NDK=$(ANDROID_NDK) \
	-DCMAKE_TOOLCHAIN_FILE=$(ANDROID_CMAKE_TOOLCHAIN) \
